...
class ECDH extends Algorithm {
  ...
  generateKey (params, extractable, usages) {
    ...
    if (!namedCurve) {
      throw new DataError('namedCurve is a required parameter for ECDH')
    }
    ...
    // 4. Set algorithm be a new ECDH
    let algorithm = new ECDH(params)

    // 5-6. Set name to ECDH
  ...
  importKey (format, keyData, algorithm, extractable, keyUsages) {
    ...
    // 2.3. "jwk" format
      ...
      key.algorithm = new ECDH(algorithm)
    }
    // 2.4. "raw" format
    else if (format === 'raw') {
      throw new CurrentlyNotSupportedError(format,'jwk')
