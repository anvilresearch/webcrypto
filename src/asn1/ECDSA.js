...
var ECDSAPrivateKey = asn.define('ECDSAPrivateKey', function () {
  this.seq().obj(
...
function decodeECDSAPublic(buffer, extras) {
  var key = ECDSAPublicKey.decode(buffer, 'der')
...
function decodeECDSAPrivate(buffer, extras) {
  var key = ECDSAPrivateKey.decode(buffer, 'der')
  var e = pad(key.e.toString(16))
  var jwk = {
    kty: 'RSA',
    n: bn2base64url(key.n),
...
function decodePrivate(buffer, extras) {
  ...
  return decodeECDSAPrivate(info.privateKey.data, extras)
}

function getDecoder(header) {
  var match = /^-----BEGIN (EC )?(PUBLIC|PRIVATE|PARAMETERS) KEY-----$/.exec(header)
  if (!match) { return null }
  var isECDSA = !!(match[1])
  var isPrivate = (match[2] === 'PARAMETERS')
  if (isPrivate) {
    return isECDSA ? decodeECDSAPrivate : decodePrivate
  }
  else {
    return isECDSA ? decodeECDSAPublic : decodePublic
  }
}
...
function jwk2pem(json) {
  ...
  if (isPrivate) {
    ...
    data = ECDSAPrivateKey.encode(jwk, 'der')
  }
  else {
    data = ECDSAPublicKey.encode(jwk, 'der')
  }
